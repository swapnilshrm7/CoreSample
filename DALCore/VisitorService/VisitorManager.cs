...
namespace VisitorService
{
    public class VisitorManager : IVisitor
    {
        ...
        public List<VisitorsData> GetVisitorLogByNameAndDate(string nameOfUser, string fromDate, string toDate, string fromTime, string toTime)
        {
            try
            {
                List<VisitorsData> allLogsByDateProvided = GetVisitorLogByDate(fromDate, toDate, fromTime, toTime);
                IEnumerable<VisitorsData> allLogsByDateAndName = allLogsByDateProvided.Where(entry => entry.NameOfVisitor.Equals(nameOfUser));
